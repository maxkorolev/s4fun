package com.maxkorolev.recursion.whisky.util
...
object StringUtil {
  ...
  def suggestionList(input: String, options: Seq[String]): Seq[String] = {
    ...
    options
      ...
      .filter(
        opt => opt._2 <= math.max(math.max(inputThreshold, opt._1.length / 2), 1)
      )
      .sortBy(_._2)
      .map(_._1)
  }
  ...
  def blockStringValue(rawString: String): String = {
    ...
    val commonIndentLines = lineSizes.drop(1).collect {
      case (line, size) if size != line.length => size
    }
    val strippedLines =
    ...
    val trimmedLines =
      strippedLines.reverse.dropWhile(isBlank).reverse.dropWhile(isBlank)
